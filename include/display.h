...
void displayTrajectory(Eigen::MatrixXd coeff, bool animate)
{
  ...
  for(int s = 0; s < (point_num - 1); s++)
  {
    ...
    getPositionFromCoeff(pos, coeff, s, 0); //获取第s段轨迹在t=0时的位置
    ...
    p.lifetime = ros::Duration(7.0); //How long the object should last before being automatically deleted.  0 means forever

    setpoint_pub.publish(p);

    // show path
    _t = my_time(s); //获取第s段分配的时间
    for(float t = 0; t < _t; t += 0.2)
    {
      Eigen::Vector3d pos(3);
      getPositionFromCoeff(pos, coeff, s, t); //获取第s段轨迹在t时的位置
  ...
  for(int s = 0; s < (point_num - 1); s++)
  {
    _t = my_time(s); //s表示第s段轨迹
    for(float t = 0; t < _t; t += 0.05)
    {
      visualization_msgs::Marker p;
      p.header.frame_id = "world";
      p.header.stamp = ros::Time::now();
      p.id = 1; //与命名空间联合起来，形成唯一的id，这个唯一的id可以将各个标志物区分开来，使得程序可以对指定的标志物进行操作
